...
def simple_dmc(wf, ham, tau, pos, g,nstep=1000, N=5, L=10):
  ...
  """
    ...
    for istep in range(nstep):
        ...
        #weight = weight* np.exp(np.sum(tau*1j*g * fp/kcopy*rho,axis=0)) #sum over all k's; from exp(-t*H_{eph}) |f>
        #weight = weight* np.exp(np.sum( np.conj(h_ks)*(f2p - f_ks),axis=0)) #from exp(-t*H_{ph}) |f>; weights are normalized
...
if __name__ == "__main__":
    ...
    nconfig = 1 #default is 5000, we only need one since there's no randomness/branching going on yet
    dfs = []
    N = 10 #num of momenta
